...
is( struct_encode( [ 'items[4]' => 'L' ], { items => [ 22, 33 ] } ), pack('LLLL', 22, 33 ), 'Static array, missing elements' );
is( struct_encode( [ 'items[4]' => 'L' ], { items => [ 22, 33, 44, 55, 66, 77 ] } ), pack('LLLL', 22, 33, 44, 55 ), 'Static array, outnumbered elements' );
is( struct_encode( [ c => 'C', 'items[c]' => 'L' ], { c => 4, items => [ 22, 33 ] } ), pack('CLLLL', 4, 22, 33 ), 'Dynamic array, static counts, missing elements' );
is( struct_encode( [ c => 'C', 'items[c]' => 'L' ], { c => 4, items => [ 22, 33, 44, 55, 66, 77 ] } ), pack('CLLLL', 4, 22, 33, 44, 55 ), 'Dynamic array, static counts, outnumbered elements' );
